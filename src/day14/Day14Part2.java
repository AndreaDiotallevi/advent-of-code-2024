...
public class Day14Part2 {
    ...
    public long processFile() {
        try {
            ...
            while (!exit) {
                Map<Point, List<Robot>> robotsFinalPositions = new HashMap<>();
                System.out.println("Seconds: " + seconds);
                for (Robot robot : robots) {
                    int x = (robot.location.x + robot.velocity.x * seconds) % sizeX;
                    int y = (robot.location.y + robot.velocity.y * seconds) % sizeY;
                    x = x < 0 ? sizeX + x : x;
                    y = y < 0 ? sizeY + y : y;
